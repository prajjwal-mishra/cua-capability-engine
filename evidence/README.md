...
| [`05-replay-recovers-from-transient`](./05-replay-recovers-from-transient) | A transient 503 on the member page. Detected as a host fault, the frame is re-requested, and the interrupted step is retried — not blamed on the recording. | `success — savings_balance=$8,241.17` | `0` |
...
| [`20-overlay-from-handoff`](./20-overlay-from-handoff) | The human click from the live handoff, compiled into a proposed overlay against the base artifact. A document to review, not an applied patch. | `proposed overlay from captured human actions` | — |
| [`21-emit-playwright`](./21-emit-playwright) | Code generation: the recorded ladder projected into a Playwright page object. Replay still executes the JSON. | `Playwright page object emitted from the artifact` | — |
